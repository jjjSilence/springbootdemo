...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    List<Student> findStudentByFirstNameEqualsAndAge(String firstName, Integer age);
    ...
    List<Student> findStudentByFirstNameAndAgeIsGreaterOrEqual(String firstName, Integer age);

    @Query(value = "SELECT * FROM student WHERE first_name = ?1 AND age >= ?2", nativeQuery = true)
    List<Student> findStudentByFirstNameAndAgeIsGreaterOrEqualNative(String firstName, Integer age);

    @Transactional(readOnly = true)
    @Query(value = "SELECT * FROM student WHERE first_name = :firstName AND age >= :age", nativeQuery = true)
    List<Student> findStudentByFirstNameAndAgeIsGreaterOrEqualNative2(
